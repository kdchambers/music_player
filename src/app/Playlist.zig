    ...
    if (storage_opt) |storage| {
        if (current_track_index_opt) |current_track_index| {
            ...
            const absolute_path = storage.entries()[new_track_index].absolutePathZ() catch |err| {
                std.log.err("Failed to create absolute path for playlist index {d}. Error -> {}", .{ new_track_index, err });
                return;
            };
            ...
            audio.mp3.playFile(std.heap.c_allocator, absolute_path) catch |err| {
                std.log.err("Failed to play track index {d} -> {}", .{ new_track_index, err });
            };
...
fn playIndex(index: u16) void {
    ...
    if (audio.output.getState() != .stopped) {
        audio.input_event_buffer.add(.stop_requested) catch |err| {
            std.log.err("Failed to send stop_requested to audio module. Error -> {}", .{err});
            return;
        };
        const max_loop_count: u32 = 1000;
        var i: u32 = 0;
    ...
    const absolute_path = storage_opt.?.entries()[index].absolutePathZ() catch |err| {
        std.log.err("Failed to create absolute path for playlist index {d}. Error -> {}", .{ index, err });
        return;
    };
    audio.mp3.playFile(std.heap.c_allocator, absolute_path) catch |err| {
        std.log.err("Failed to play track index {d} -> {}", .{ index, err });
    };
...
pub fn doAction(index: event_system.ActionIndex) void {
    ...
    if (index >= Command.init_and_play_audio_command_base and index < init_and_play_max) {
        ...
        output_events.add(.playlist_initialized) catch |err| {
            std.log.err("Failed to add .playlist_initialized event to Playlist event queue -> {}", .{err});
        };
        ...
        switch (index) {
            Command.track_next => trackNext() catch |err| std.log.err("Failed to play next track. Error -> {}", .{err}),
            Command.track_previous => trackPrevious() catch |err| std.log.err("Failed to play previous track. Error -> {}", .{err}),
            Command.track_pause => trackPause() catch |err| std.log.err("Failed to pause track. Error -> {}", .{err}),
            Command.track_resume => trackResume() catch |err| std.log.err("Failed to resume track. Error -> {}", .{err}),
            else => {
...
pub const Storage = struct {
    ...
    pub fn create(
        ...
        const source_parent_path = try directory.dir.realpath(".", output_buffer[0..]);
        ...
        while (try iterator.next()) |entry| {
            ...
            if (entry.kind == .File) {
                ...
                if (matchExtension("MP3", extension) or matchExtension("FLAC", extension)) {
                    ...
                    var file = directory.dir.openFile(entry.name, .{}) catch |err| {
